...
class Flow:
    ...
    def get_data(self) -> dict:
        ...
        data = {
            ...
            "flow_duration": float(1e6 * packet_time.get_duration()),
            "flow_byts_s": float(flow_bytes.get_rate()),
            "flow_pkts_s": float(packet_count.get_rate()),
            "fwd_pkts_s": float(packet_count.get_rate(PacketDirection.FORWARD)),
            "bwd_pkts_s": float(packet_count.get_rate(PacketDirection.REVERSE)),
            ...
            # Response Time
            "down_up_ratio": float(packet_count.get_down_up_ratio()),
            "pkt_size_avg": float(packet_length.get_avg()),
        ...
        payloads_sample = np.pad(payloads, (0, N - len(payloads)), 'constant')
